...
func (f *Framework) CreateEtcd(obj *api.Etcd) error {
	_, err := f.extClient.Etcds(obj.Namespace).Create(obj)
	return err
}

func (f *Framework) GetEtcd(meta metav1.ObjectMeta) (*api.Etcd, error) {
	return f.extClient.Etcds(meta.Namespace).Get(meta.Name, metav1.GetOptions{})
}

func (f *Framework) PatchEtcd(meta metav1.ObjectMeta, transform func(*api.Etcd) *api.Etcd) (*api.Etcd, error) {
	etcd, err := f.extClient.Etcds(meta.Namespace).Get(meta.Name, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}
	etcd, _, err = util.PatchEtcd(f.extClient, etcd, transform)
	return etcd, err
}

func (f *Framework) DeleteEtcd(meta metav1.ObjectMeta) error {
	return f.extClient.Etcds(meta.Namespace).Delete(meta.Name, deleteInBackground())
}

func (f *Framework) EventuallyEtcd(meta metav1.ObjectMeta) GomegaAsyncAssertion {
	return Eventually(
		func() bool {
			_, err := f.extClient.Etcds(meta.Namespace).Get(meta.Name, metav1.GetOptions{})
			if err != nil {
...
func (f *Framework) CleanEtcd() {
	etcdList, err := f.extClient.Etcds(f.namespace).List(metav1.ListOptions{})
	if err != nil {
		return
	}
	for _, e := range etcdList.Items {
		if _, _, err := util.PatchEtcd(f.extClient, &e, func(in *api.Etcd) *api.Etcd {
	...
	if err := f.extClient.Etcds(f.namespace).DeleteCollection(deleteInBackground(), metav1.ListOptions{}); err != nil {
