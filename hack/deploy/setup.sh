...
if [ "$SELF_HOSTED" -eq 1 ]; then
  echo "${KUBEDB_SCRIPT}hack/deploy/kubedb.sh | bash -s -- --operator-name=etcd-operator $ARGS"
  ${KUBEDB_SCRIPT}hack/deploy/kubedb.sh | bash -s -- --operator-name=etcd-operator ${ARGS}
fi
...
if [ "$MINIKUBE" -eq 1 ]; then
  ...
  cat $CLI_ROOT/hack/deploy/kubedb-catalog/etcd.yaml | $ONESSL envsubst | kubectl apply -f - || true
